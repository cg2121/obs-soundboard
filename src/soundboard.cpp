...
#define QT_STD(str) QString::fromStdString(str)
#define QT_TO_STD(str) str.toStdString()
#define QTStr(str) QString(obs_module_text(str))
#define MainStr(str) QString(obs_frontend_get_locale_string(str))

static std::string GetDefaultString(std::string name = "")
{
	if (name.empty())
		name = obs_module_text("Sound");
	...
	for (;;) {
		std::string out = name + " " + std::to_string(i);
...
MediaObj *Soundboard::GetCurrentMediaObj()
{
	...
	std::string uuid = QT_TO_STD(item->data(Qt::UserRole).toString());
...
void Soundboard::CreateSource()
{
	if (obs_obj_invalid(source)) {
		source = obs_source_create(
			"ffmpeg_source",
			obs_module_text("Soundboard"), nullptr,
			nullptr);
...
OBSDataArray Soundboard::SaveMedia()
{
	...
	for (int i = 0; i < ui->list->count(); i++) {
		...
		obs_data_set_string(settings, "name", obj->GetName().c_str());
		obs_data_set_string(settings, "path", obj->GetPath().c_str());
		obs_data_set_bool(settings, "loop", obj->LoopEnabled());
		obs_data_set_double(settings, "volume", (double)obj->GetVolume());

		//OBSDataArrayAutoRelease array = obs_hotkey_save(hotkey);
		//obs_data_set_array(settings, "sound_hotkey", array);
...
//		OBSDataArrayAutoRelease array = obs_data_get_array(settings, "sound_hotkey");
//		obs_hotkey_load(hotkey, array);
...
void Soundboard::Save(OBSData saveData)
{
	...
	if (obj)
		obs_data_set_string(saveData, "current_sound", obj->GetName().c_str());
...
void Soundboard::Load(OBSData saveData)
{
	...
	std::string lastSound = obs_data_get_string(saveData, "current_sound");
...
void Soundboard::Add(const std::string &name_, const std::string &path, bool loop)
{
	std::string name = GetDefaultString(name_);

	MediaObj *obj = new MediaObj(name, path);
	obj->SetLoopEnabled(loop);

	QListWidgetItem *item = new QListWidgetItem(QT_STD(name));
	item->setData(Qt::UserRole, QT_STD(obj->GetUUID()));
...
void Soundboard::on_actionAdd_triggered()
{
	...
	auto added = [&, this]() {
		std::string name = QT_TO_STD(edit.GetName());
		std::string path = QT_TO_STD(edit.GetPath());
	...
	edit.SetName(QT_STD(GetDefaultString()));
	edit.exec();
}
...
void Soundboard::on_actionEdit_triggered()
{
	...
	edit.SetName(QT_STD(obj->GetName()));
	edit.SetPath(QT_STD(obj->GetPath()));
...
void Soundboard::on_actionRemove_triggered()
{
	...
	QString name = QT_STD(obj->GetName());
...
void Soundboard::on_actionDuplicate_triggered()
{
	...
	std::string name = GetDefaultString(obj->GetName());
	std::string path = obj->GetPath();
